...
- 系统性能采样也由原生采样器负责，PowerShell 只保留启动、监控状态和收尾，减少游戏中监控脚本本身的 CPU 占用。
- 游戏运行时可进入一次轻量模式，退出后按快照恢复；轻量模式只调整优先级，不结束后台进程。
...
帧时间由 PresentMon 按实际帧记录，不依赖低频轮询。后台进程采样由 `FrameScopeProcessSampler.exe` 执行，默认每 100ms 记录一次全进程 CPU、内存和 IO 数据；采样器使用原生实现并跟随监控父进程退出，避免旧版 PowerShell 高频全进程扫描导致监控器自身占用偏高或退出残留。
...
- `FrameScopeMonitor.exe`：主界面。
- `FrameScopeProcessSampler.exe`：低占用后台进程采样器。
- `FrameScopeWatcher.ps1`：自动监听游戏启动与退出。
- `Monitor-CS2-HighFreq.ps1`：单次监测流程编排。
