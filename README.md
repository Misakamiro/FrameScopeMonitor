...
- 图表绘制新增 screen-space 合并：不同时间点如果落到同一个 canvas/screen x 像素，会先合并成一个绘制点，避免 Vcore 和后台进程图出现同屏 x 竖向伪影。
- `null` gap 不再被 `Number(null)` 误当成 `0`，CPU Voltage / Vcore 面积图不会被采集 gap 拉到 0；Vcore duplicate screen x 和 same-x vertical 均已验证为 `0`。
- 后台进程图保持 line-only，不再用面积填充污染视图；真实短 spike 仍会保留。
- 真实低功耗状态仍会保留，例如 GPU `225 MHz` 这类有效低 P-state 不会被误删。
- AMD LibreHardwareMonitor `/amdcpu/0/voltage/2..9` Core VID 现在按来源拒绝，不再使用 `0.7V` 阈值策略；错误的约 `0.5V` VID 不再显示，约 `1.08V` 的 Vcore 也不会冒充 `CPU Core VID`。
- CPU Core VID tab 会保留。当 VID 来源不可信时，报告会显示中文说明并引导查看 `CPU Voltage / Vcore`，不再留下空白图表。
...
- 发布前验证包含前端 verify、原生测试、chart sampling、layout probe JSON 解析、`git diff --check` 和残留进程检查。
- 已知边界：本轮没有重新安装 FrameScope，没有启动真实游戏，没有进行 BF6 真实游戏测试。
...
- AMD LibreHardwareMonitor 在部分 AMD 平台上可能给出不可信的 Core VID 来源，例如 `/amdcpu/0/voltage/2..9`；FrameScope 会按来源拒绝这些 VID，不再依赖 `0.7V` 阈值判断。
- 如果你看到约 `1.08V` 的读数，它通常属于 SuperIO Vcore / CPU Voltage 口径；FrameScope 会保留在 `DATA.cpuVoltage`，不会填进 `DATA.cpuVid`。
- 当 CPU Core VID 来源不可信时，`CPU Core VID` tab 仍会保留，并显示中文说明：这不是漏画图，而是该 VID 来源已被拒绝；请查看 `CPU Voltage / Vcore` 图表确认真实整体电压。
...
本轮发布前完成过只读和自动化验证，结论为 PASS：

- `tools\Run-Frontend.ps1 verify` 通过。
- 原生测试重建和核心测试通过。
- chart sampling 覆盖 screen-space duplicate x、same-x vertical、Vcore gap、后台进程 line-only spike 保留和合法 VID fixture。
- layout probe 输出的 `report-overflow-probe.json` 可由 Windows PowerShell 默认 `Get-Content -Raw | ConvertFrom-Json` 解析，且 `allNoOverflow=true`。
- `git diff --check` 通过。
- 残留进程检查输出 `NO_MATCHING_RESIDUAL_PROCESSES`。
- 报告资源保持 `bucketMs=1000`、`DATA.cpuVoltage` 和 `DATA.cpuVid`。

本轮没有重新安装 FrameScope，没有启动真实游戏，也没有做 BF6 真实游戏验收。
