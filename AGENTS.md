...
- 必须保持任务上下文和已确认的约束，不得在后续步骤中遗忘或擅自扩大范围。
- 回复语气要干练，但说明具体变更时必须逐步、详细且易懂地说明执行了什么、为什么这样改。
- 每次代码变更完成前，必须从头到尾测试和检查本轮修改过的全部步骤、功能与 bug；不能只验证最后一个文件或只凭编译成功判断完成。
- 涉及用户本机已安装版本时，必须同步更新安装目录或重新安装新包后才算完成。只有当前用户在当前任务中明确要求“仅本地源码”“暂不安装”或“暂不推送”时，才遵守对应限制；后续任务若得到明确安装或 push 授权，应按新授权执行，不得把旧任务限制永久化。
- 涉及运行链路时，必须验证启动、监听、采样、报告生成、按配置自动打开报告，以及退出后无残留进程或专用采集会话。
- 修改安装器时，必须验证首次安装、覆盖安装、卸载、保留用户数据和删除用户数据两条路径。
- 重新打包后必须核对最终 payload，确认其中是本轮构建的 exe、前端资源、依赖和说明文件，不能只看 build 命令退出码。
- 交付时必须逐项说明：改动文件、实现的功能或修复的 bug、是否更新本机安装版、是否重新打包、执行过的测试、未能执行的验证、残留风险以及回滚方式。
...
GameLite 核心实现位于独立项目；本仓库仍保留 7 个参数转发 compatibility wrappers：`Install-GameLiteAutoTrigger.ps1`、`Check-GameLiteAutoTrigger.ps1`、`Remove-GameLiteAutoTrigger.ps1`、`GameLiteSession.ps1`、`Enter-GameLite.ps1`、`Exit-GameLite.ps1`、`Invoke-GameLiteSGuardThrottle.ps1`，以及 3 个 cmd launchers：`Install-GameLiteAutoTrigger.cmd`、`Check-GameLiteAutoTrigger.cmd`、`Remove-GameLiteAutoTrigger.cmd`。它们是兼容桥，不属于 FrameScope 生产链路，也不会被普通 build 或安装器部署。

`tests/lightweight-separation-tests.ps1` 验证 wrapper 转发、独立项目脚本存在，以及 FrameScope 生产源码与 packaging 不含 GameLite 自动触发实现。没有用户明确授权时，不执行任何 GameLite WMI 安装、移除或真实游戏操作。
