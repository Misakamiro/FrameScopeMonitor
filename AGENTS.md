...
本仓库不包含任何 GameLite 执行入口、兼容包装器或独立项目路径。`tests/lightweight-separation-tests.ps1` 只验证 FrameScope 生产源码、build 与 packaging 不含 GameLite 自动触发实现，不依赖独立 GameLite 项目。没有用户明确授权时，不执行任何 GameLite WMI 安装、移除或真实游戏操作。
